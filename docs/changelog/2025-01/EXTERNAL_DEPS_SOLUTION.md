...
I've created a solution using **external npm packages** instead of custom
scripts with only built-in modules.
...
1. **`scripts/generate-meta-with-deps.ts`** - New script using external
   dependencies
