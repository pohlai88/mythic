...
This solution uses **established npm packages** to handle file watching and navigation generation for Nextra, instead of custom scripts with only built-in modules.
...
The new script (`generate-meta-with-deps.ts`) replaces the old one (`generate-meta.ts`):
