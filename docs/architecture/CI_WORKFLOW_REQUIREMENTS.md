...
**Status**: ✅ Active | **Last Updated**: 2026-01-11 **Purpose**: Document
what's required vs optional in the CI workflow
...
**Purpose**: Full build and validation **Runs on**: All pushes and PRs
**Steps**:
...
**Purpose**: Quality checks in parallel **Runs on**: All pushes and PRs
**Steps**:
...
**Purpose**: Build only changed files (faster PR checks) **Runs on**: Pull
requests only **Condition**: `if: github.event_name == 'pull_request'`
**Steps**:
...
**Status**: ⚠️ **Expected warning** - Can be ignored **Reason**: Linter can't
verify secrets exist **Impact**: None - workflow works correctly **Solution**:
Ignore the warning, or set the secrets if you want remote cache

### Issue: Workflow fails on "Validate verbatim configs"

**Solution**: Run `pnpm validate:verbatim` locally to see what's wrong **Fix**:
Update config files to match verbatim templates in
