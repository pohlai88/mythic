...
**Status**: ✅ Active | **Last Updated**: 2026-01-11 **Purpose**: Document all
environment variables used in the monorepo
...
- **`.env`** - Base environment variables (committed to git if using
  `.env.example`)
...
### Using Boardroom Validated Env (apps/boardroom/src/lib/env.ts)

**Note**: Root `lib/env.ts` has been removed. Use app-specific validated env:

```typescript
// In Boardroom app (uses Zod-validated env)
import { env, getDatabaseUrl } from "@/src/lib/env"
...
- ❌ Expose secrets in client-side code (use `NEXT_PUBLIC_*` only for public
  values)
