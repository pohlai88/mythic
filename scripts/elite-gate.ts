...
async function runCheck(command: string, name: string): Promise<{ success: boolean; error?: string }> {
  try {
...
async function runEliteGate(): Promise<EliteGateResult> {
  ...
  logger.info({ success: tokenCheck.success }, tokenCheck.success ? 'Tokens valid' : 'Tokens invalid')
  ...
  logger.info({ success: serverCheck.success }, serverCheck.success ? 'Server Component ratio > 70%' : 'Server Component ratio < 70%')
