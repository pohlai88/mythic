...
// Valid status values
const VALID_STATUS = ["draft", "active", "sealed", "legacy", "archived"]
...
function checkNamingConvention(file: string): CleanlinessIssue[] {
  ...
  if (!hasValidPattern) {
    issues.push({
      ...
      recommendation:
        "Use format: DOC-XXXX_descriptive-name.md, [hash-8]_name.md, or v[VERSION]_name.md",
    })
  }
...
async function main() {
  ...
  const compliance = Math.max(
    0,
    Math.min(100, ((totalPossibleIssues - allIssues.length) / totalPossibleIssues) * 100)
  )
  ...
  console.log(
    `Compliance Score: ${compliance >= 90 ? chalk.green(compliance.toFixed(1) + "%") : compliance >= 70 ? chalk.yellow(compliance.toFixed(1) + "%") : chalk.red(compliance.toFixed(1) + "%")}\n`
  )
  ...
  if (bySeverity.critical > 0 || bySeverity.high > 0) {
    console.log(
      chalk.red("❌ Cleanliness audit failed. Critical or high-priority issues detected.\n")
    )
    process.exit(1)
  }
