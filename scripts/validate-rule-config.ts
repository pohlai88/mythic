...
function validateRule(ruleFile: RuleFile, autoFix: boolean): ValidationResult {
  ...
  if (!/^\d{3}_[\w-]+\.mdc$/.test(ruleFile.filename)) {
    result.errors.push(`Invalid filename format. Expected: [NNN]_descriptive-name.mdc, got: ${ruleFile.filename}`);
  }
  ...
  if (fm.alwaysApply === true) {
    if (!ALLOWED_ALWAYS_APPLY.includes(ruleFile.filename)) {
      if (autoFix) {
        ...
        const newContent = ruleFile.content.replace(
          /alwaysApply:\s*true/,
          'alwaysApply: false'
        );
...
async function validateRules(autoFix: boolean, strict: boolean) {
  ...
  const alwaysApplyCount = rules.filter(
    r => r.frontmatter?.alwaysApply === true
  ).length;
  
  console.log(`📊 Summary:`);
  console.log(`   Total rules: ${rules.length}`);
  console.log(`   Always loaded: ${alwaysApplyCount} (target: 3)`);
  console.log(`   Conditional: ${rules.length - alwaysApplyCount}`);
  
  if (alwaysApplyCount > 3) {
    console.log(`   ⚠️  WARNING: ${alwaysApplyCount - 3} too many alwaysApply rules`);
  }
