...
function validateFile(filePath: string): ValidationResult[] {
  ...
  if (content.includes("from 'zod'") || content.includes('from "zod"')) {
    const lineIndex = lines.findIndex(
      (line) => line.includes("from 'zod'") || line.includes('from "zod"')
    )
    if (lineIndex >= 0) {
      results.push({
        file: filePath,
  ...
  if (
    content.includes("import { z } from 'zod/v4'") ||
    content.includes('import { z } from "zod/v4"')
  ) {
    const lineIndex = lines.findIndex(
      (line) =>
        (line.includes("import { z } from 'zod/v4'") ||
          line.includes('import { z } from "zod/v4"')) &&
  ...
  if (content.includes("zod/v4") && !content.includes("z as z4") && content.includes("z.")) {
    const lineIndex = lines.findIndex((line) => line.includes("z.") && !line.includes("z4."))
    if (lineIndex >= 0) {
      results.push({
        file: filePath,
        line: lineIndex + 1,
