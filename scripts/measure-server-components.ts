...
function extractRoute(filePath: string): string {
  ...
  const match = filePath.match(/apps\/[^/]+\/app\/(.+?)\/(page|layout|loading|error|not-found)\.tsx$/)
  if (match) {
    return `/${match[1]}`
  }
...
async function main() {
  ...
  const overallRatio = totalServer + totalClient > 0 
    ? totalServer / (totalServer + totalClient) 
    : 0
  ...
  if (overallRatio >= 0.7) {
    ...
    console.log(`⚠️  Target not met: Server Component ratio is ${(overallRatio * 100).toFixed(1)}%\n`)
  }
  ...
  metrics.forEach(metric => {
    ...
    console.log(`   Server: ${metric.serverComponents}, Client: ${metric.clientComponents}, Ratio: ${(metric.ratio * 100).toFixed(1)}%`)
  })
