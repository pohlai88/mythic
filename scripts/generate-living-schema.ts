...
function generateZodSchemas() {
  ...
  for (const { name } of tables) {
    ...
    output.push(
      `  ${typeName}Insert = z4.infer<typeof insert${typeName.charAt(0).toUpperCase() + typeName.slice(1)}Schema>,`
    )
    output.push(
      `  ${typeName}Select = z4.infer<typeof select${typeName.charAt(0).toUpperCase() + typeName.slice(1)}Schema>,`
    )
  }
  output.push("}")
