...
function verifyFile(file: string): VerificationResult {
  ...
  if (!hasFrontmatter) {
    ...
    const required = ['doc_type', 'status', 'owner', 'source_of_truth', 'created', 'modified', 'tags']
...
async function main() {
  ...
  console.log('\nCompliance:')
  console.log(`  Frontmatter: ${withFrontmatter}/${total} (${Math.round((withFrontmatter/total)*100)}%)`)
  console.log(`  Naming: ${withCorrectNaming}/${total} (${Math.round((withCorrectNaming/total)*100)}%)`)
