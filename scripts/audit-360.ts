      ...
      while ((match = importRegex.exec(content)) !== null) {
        ...
        if (importPath.startsWith("@/")) {
          ...
          if (!exists && !importPath.includes("/*") && !importPath.endsWith("*")) {
            ...
            const fileExists =
              existsSync(join(rootDir, baseUrl, relativePath)) ||
...
function auditRouteHandlers() {
  ...
  for (const routeFile of routeFiles) {
    ...
    try {
      ...
      const hasGet =
        content.includes("export async function GET") || content.includes("export const GET")
      const hasPost =
        content.includes("export async function POST") || content.includes("export const POST")
      const hasPut =
        content.includes("export async function PUT") || content.includes("export const PUT")
      const hasDelete =
        content.includes("export async function DELETE") || content.includes("export const DELETE")
      const hasPatch =
        content.includes("export async function PATCH") || content.includes("export const PATCH")
...
function auditTypeScriptConfig() {
  ...
  const tsconfigFiles = [
    ...
    "packages/NextJS/Shared-Utils/tsconfig.json",
    "packages/TypeScript/Shared-Types/tsconfig.json",
    "packages/TailwindCSS-V4/Design-System/tsconfig.json",
  ]
...
function auditNextConfig() {
  ...
  const nextConfigFiles = ["apps/boardroom/next.config.mjs", "apps/docs/next.config.mjs"]
...
function auditEnvironmentVariables() {
  ...
  const examplePatterns = [/example|placeholder|test|demo|sample|dummy|fake|your-|TODO|FIXME/i]
