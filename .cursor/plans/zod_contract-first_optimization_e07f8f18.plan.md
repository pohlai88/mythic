---
name: Zod Contract-First Optimization
overview: Transform the codebase to use Zod as the single source of truth for all contracts (API, forms, env, DB, types). Maximize Zod feature utilization from 28% to 85%+ and integrate Drizzle-Zod for synergistic database validation.
todos:
  - id: env-validation
    content: Create environment variable validation schema (apps/boardroom/src/lib/env.ts)
    status: completed
  - id: form-validation
    content: Replace manual validation in codex/index.ts with Zod schemas and React Hook Form
    status: completed
  - id: api-response-validation
    content: Add response validation schemas to all server actions
    status: completed
  - id: query-params-validation
    content: Create query parameter schemas for all API endpoints
    status: pending
  - id: drizzle-refinements
    content: Add .refine() and .pipe() to Drizzle-generated schemas
    status: pending
  - id: safe-parse-migration
    content: Replace all .parse() with .safeParse() for better error handling
    status: pending
  - id: advanced-features
    content: "Implement advanced Zod features: .refine(), .pipe(), .catch(), .readonly()"
    status: pending
  - id: update-drizzle-config
    content: Update drizzle.config.ts to use validated environment variables
    status: completed
  - id: update-db-index
    content: Update src/db/index.ts to use validated environment variables
    status: completed
  - id: nextjs-server-actions-validation
    content: Create Server Actions validation wrapper (apps/boardroom/src/lib/actions/validate-action.ts)
    status: pending
  - id: nextjs-route-handlers-validation
    content: Create Route Handler validation wrapper (apps/boardroom/src/lib/api/route-handler.ts)
    status: pending
  - id: nextjs-cache-validation
    content: Create validated cache wrapper (apps/boardroom/src/lib/cache/validated-cache.ts)
    status: pending
  - id: nextjs-middleware-validation
    content: Add Zod validation to middleware (apps/boardroom/src/middleware.ts)
    status: pending
  - id: nextjs-form-actions-validation
    content: Create validated form action hook (apps/boardroom/src/lib/forms/validated-form-action.ts)
    status: pending
  - id: react-zod-hooks
    content: Create React hooks with Zod validation (useZodForm, useZodState, useZodContext)
    status: pending
  - id: primereact-zod-forms
    content: Create PrimeReact form components with Zod validation (ZodFormField, ZodDataTable, ZodDialogForm)
    status: pending
  - id: tailwind-zod-styling
    content: Create Tailwind error styling utilities tied to Zod validation (zod-error-styles, validation-states)
    status: pending
  - id: react-prop-validation
    content: Create React component prop validation wrapper (withZodProps)
    status: pending
  - id: react-context-validation
    content: Create React context validation utilities (createZodContext)
    status: pending
  - id: user-global-config-validation
    content: Create comprehensive User/Global Config validation schemas with security enforcement
    status: pending
  - id: encryption-validation
    content: Create encryption operation validation schemas (keys, encrypted data, decryption requests)
    status: pending
  - id: rbac-validation
    content: Create RBAC permission validation schemas and service
    status: pending
  - id: frontend-customization-schemas
    content: Create frontend customization schemas (layout, view, component configs)
    status: pending
  - id: websocket-validation
    content: Create WebSocket message validation schemas for real-time communication
    status: pending
  - id: audit-trail-validation
    content: Create immutable audit trail validation schemas (6W1H events)
    status: pending
  - id: dynamic-form-generation
    content: Create dynamic form generation from stencils with Zod validation
    status: pending
---
