...
- Amendment **A-001** (Courts by Office, interpretation notes attribution) — Ratified
...
3) **Public Accessibility:** Read access is granted to all stakeholders governed by the Constitution.
4) **Attribution:** Every entry is attributable to an Office + Team (or actor identity).
5) **Reproducibility:** Evidence references must be immutable artifacts.
6) **Deterministic Integrity:** Canonical serialization is required before hashing.
...
- `artifact` (object)
  - `artifact_type` (enum) — `PR | COMMIT | BUILD | DEPLOY | RUNTIME_REQUEST | AUDIT_SCAN | DOC`
  - `artifact_ref` (string) — immutable reference (PR number, commit hash, build id, doc path+hash)
...
- Inputs MUST include:
  - *LBOS Origin Paper (Sealed)* (immutable hash ref)
  - *NexusCanon Constitution v1.0* (immutable hash ref)
...
- Secrets MUST NOT be embedded; store elsewhere and reference immutable artifacts.
...
This spec is ratified. Operational parameters (identity scheme details, snapshot cadence, size limits) are specified in **Annex A** and may evolve additively via amendment.
