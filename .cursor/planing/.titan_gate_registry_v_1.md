...
**Status:** Operational Pack (Derived) **Derived From:** Titan Protocol v1.0
(Ratified) **Purpose:** Provide a single, canonical registry of Titan Gate IDs
and a drop-in CI/PR checklist format.
...
> Copy/paste into your PR template. This is designed to be human-reviewable and
> CI-verifiable.
...
- [ ] Core write-path module changed (`accounting-core/`, `compliance-core/`,
      `audit-core/`)
...
- Clauses impacted:
  `I.__, II.__, III.__, IV.__, V.__, VI.__, VII.__, VIII, IX.__`
...
- [ ] If a gate fails and the change is still desired, I will propose an
      Amendment per A-000.
...
This format is intended for CI to print and for humans to read. Each gate run
should output the following section:
...
- **AUDIT gates** run post-deploy on a schedule and escalate WARN→VETO per Titan
  policy.
