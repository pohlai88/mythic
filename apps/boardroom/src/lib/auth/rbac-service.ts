...
export async function checkPermission(
  ...
  if (!validatedResult.success) {
    throw new Error(`Invalid permission check: ${validatedResult.error.issues[0]?.message || 'Unknown error'}`)
  }
  ...
  const allowed = await isActionAllowedForRole(
    validated.action,
    userRole,
 ...
 */
async function getUserRoleInCircle(
  userId: string,
  circleId?: string
): Promise<Role> {
  if (!circleId) {
 ...
 */
export function validateCircleMembership(
  membership: unknown
): CircleMembership {
