...
export function detectConnectionMethod(): 'docker' | 'direct' | 'unknown' {
  ...
  if (dbUrl) {
    ...
    if (dbUrl.includes('amazonaws.com') || dbUrl.includes('supabase.co') || dbUrl.includes('railway.app')) {
      return 'direct'
    }
...
export async function validateDockerConnection(): Promise<ValidationResult> {
  ...
  try {
    ...
    const dbInfo = Array.isArray(dbInfoResult) && dbInfoResult[0]
    ...
    console.log(`   PostgreSQL Version: ${result.databaseInfo.version.split(' ')[0]} ${result.databaseInfo.version.split(' ')[1]}`)
...
export async function validateDirectConnection(): Promise<ValidationResult> {
  ...
  const isLocal = result.connectionString.includes('localhost') || result.connectionString.includes('127.0.0.1')
  ...
  try {
    ...
    if (isNeon) {
      ...
      if (hasChannelBinding) {
        ...
        result.warnings.push('Channel binding not enabled - consider adding channel_binding=require for enhanced security')
      }
      console.log()
    }
...
async function validateConnection(): Promise<ValidationResult> {
  ...
  console.log(`📡 Connection Method: ${method === 'docker' ? 'Docker Desktop' : method === 'direct' ? 'Direct Database' : 'Unknown'}\n`)
...
function printSetupInstructions(method: 'docker' | 'direct') {
  ...
  if (method === 'docker') {
    ...
    console.log('      DATABASE_URL=postgresql://user:pass@ep-xxx-pooler.neon.tech/db?sslmode=require')
    console.log('   4. Run: pnpm validate:db\n')
