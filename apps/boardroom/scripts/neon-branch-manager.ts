...
function createBranch(config: BranchConfig): string {
  try {
    const args = ["branches", "create", "--project-id", config.projectId, "--name", config.name]
...
function switchToBranch(config: BranchConfig): void {
  ...
  if (!envContent.includes("NODE_ENV=")) {
    ...
    envContent = envContent.replace(/NODE_ENV=.*/, `NODE_ENV=${config.environment}`)
  }
...
function listBranches(projectId: string): void {
  try {
    const output = execSync(`neonctl branches list --project-id ${projectId} --output json`, {
      encoding: "utf-8",
    })
  ...
  switch (command) {
    case "create":
      ...
      const env =
        ...
        "development"
      const projectId =
        args.find((arg) => arg.startsWith("--project-id"))?.split("=")[1] ||
