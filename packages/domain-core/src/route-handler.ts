 ...
 */
export interface RouteSchema<TParams = unknown, TQuery = unknown, TBody = unknown, TResponse = unknown> {
 ...
 */
export function createValidatedRoute<TParams = unknown, TQuery = unknown, TBody = unknown, TResponse = unknown>(
  schema: RouteSchema<TParams, TQuery, TBody, TResponse>,
  handler: RouteHandler<TParams, TQuery, TBody, TResponse>
) {
  return async (request: NextRequest, { params }: { params: Promise<Record<string, string>> | Record<string, string> }): Promise<NextResponse> => {
    try {
      ...
      const validatedParams = (schema.params
        ...
        : (resolvedParams as TParams)) as TParams
